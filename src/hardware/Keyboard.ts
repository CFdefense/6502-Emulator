...
export class Keyboard extends Hardware implements Interrupt {
    public irqNumber: number = 1;
    public priority: number = 1;
    public deviceName: string;
    public outputBuffer: string[] = [];
    public controller: InterruptController;
    private keyPressHandler: ((key: string) => void) | null = null;
    private silentMode: boolean = false;
    private debugMode: boolean = false;
    
    constructor(controllerRef: InterruptController, debugMode: boolean) {
    ...
    public onKeyPress(handler: (key: string) => void): void {
        this.keyPressHandler = handler;
    }

    public removeKeyPressListener(): void {
