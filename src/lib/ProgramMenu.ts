...
export class ProgramMenu {
    ...
    constructor(private cpu: Cpu, private interruptController: InterruptController, private debugMode: boolean) {
        // Get the MMU instance from the CPU
        this.mmu = cpu['MMU'];
        this.keyboard = new Keyboard(interruptController, debugMode);
    }
    ...
    public loadProgram(index: number): Program | null {
    ...
    public validateProgramResults(program: Program, actualRegisters: any): boolean {
    ...
    public addCustomProgram(onComplete: () => void): void {
        ...
        const isValidHexByte = (str: string): boolean => {
